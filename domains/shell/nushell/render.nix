...
let
  ...
  sshAgentText =
    if !sshEnabled then
      ...
      ''
        let ssh_sock_path = $"/run/user/(id -u)/ssh-agent.socket"

        let live = if ($env.SSH_AUTH_SOCK? == null) {
            false
        } else {
        ...
        if not $live {
            mut waited = 0
            while ((not ($ssh_sock_path | path exists)) and $waited < 50) {
                sleep 100ms
                $waited += 1
            }

            if ($ssh_sock_path | path exists) {
                $env.SSH_AUTH_SOCK = $ssh_sock_path
            } else {
                let out = (^ssh-agent -s | lines)
                $env.SSH_AUTH_SOCK = ($out | get 0 | split row ';' | get 0 | split row '=' | get 1)
            }
        }
...
[
  ...
  {
    path = "domains/shell/nushell/config/ssh-agent.nu";
    text = sshAgentText;
    checks = lib.optionals sshEnabled [
      (requireInfix sshAgentText "let ssh_sock_path"
        "nushell ssh-agent bootstrap should declare the shared agent socket path"
      )
    ];
  }
]
