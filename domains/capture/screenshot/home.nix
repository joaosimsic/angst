...
let
  ...
  targetDirExpanded = cfg.targetDir;

  angstScreenshot = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      TARGET_DIR="${targetDirExpanded}"
      # Expand $HOME if present (nix string "$HOME/..." stays literal, shell expands)
      ...
      COPY_TO_CLIPBOARD="${if cfg.copyToClipboard then "1" else "0"}"
      SAVE_TO_FILE="${if cfg.saveToFile then "1" else "0"}"
      INTERACTIVE="${if cfg.interactive then "1" else "0"}"

      # Auto detection if BACKEND=auto
      ...
      capture_portal() {
        # Try org.freedesktop.portal.Screenshot via gdbus; fallback to native
        URI=""
        if command -v gdbus >/dev/null 2>&1; then
          # portal returns (uint32 handle, string uri)
          # Use non-interactive unless INTERACTIVE=1
          OPTIONS="{}"
          if [ "$INTERACTIVE" = "1" ]; then
            OPTIONS="{'interactive': <true>}"
          fi
          # gdbus call with timeout; parse uri from output like "(uint32 0, 'file:///tmp/...')"
          ...
          URI=''${URI#file://}
          # Some portals return with url encoding
        ...
        # Fallback to native if portal fails
        if [ -n "''${WAYLAND_DISPLAY:-}" ]; then
          capture_grim "$1"
        else
          capture_maim "$1"
