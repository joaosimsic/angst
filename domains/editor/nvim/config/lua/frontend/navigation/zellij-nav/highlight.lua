...
local function highlight_tail_tokens(buf, row, line, len, start_pos)
	local iter = each_token(line, start_pos)
...
local function handle_prompt_line(buf, row, line, len)
	if not (line:match("^[%a_][%w_]*") and (line:find("~/") or line:find(" %["))) then
		return false
	end
	...
	while scan_pos <= zone_end do
		...
		if not s or s > zone_end then
			break
		end
		local e = line:find("%s", s)
		if not e or e > zone_end then
			e = zone_end + 1
		end
	...
	if last_end > 0 and last_end < len then
		highlight_tail_tokens(buf, row, line, len, last_end + 1)
	end
...
function M.apply(buf)
	...
	for i, line in ipairs(lines) do
		...
		if handle_prompt_line(buf, row, line, len) then
			goto continue
		end
