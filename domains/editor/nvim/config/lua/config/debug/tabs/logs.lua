...
local function add_entry(view, entry)
	...
	local tag = R.ellipsize(entry.tag, TAG_WIDTH)
	...
	R.add_highlight(view, line_nr, "DebugMuted", 0, 8)
	R.add_highlight(view, line_nr, level_highlights[entry.level] or "DebugValue", level_start, level_start + #level)
	R.add_highlight(view, line_nr, "DebugLabel", tag_start, tag_start + #tag)
	R.add_highlight(view, line_nr, "DebugValue", message_start, -1)

	for i = 2, #lines do
		local continuation = string.rep(" ", message_start) .. lines[i]
		local continuation_nr = R.add_line(view, continuation)
		R.add_highlight(view, continuation_nr, "DebugValue", message_start, -1)
	end
end

---@param card_width? integer
---@return { lines: string[], highlights: table[] }
function M.render(card_width)
	local view = {
		lines = {},
		highlights = {},
	}
	...
	if #entries == 0 then
		local line_nr = R.add_line(view, "No debug logs recorded yet.")
		R.add_highlight(view, line_nr, "DebugMuted", 0, -1)
		return view
	end
	...
	R.add_highlight(view, separator_nr, "DebugBorder", 0, -1)
