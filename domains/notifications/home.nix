...
let
  ...
  effectiveBackend =
    if notificationBackend == "auto" then
      if hasWayland && !hasX11 then "mako" else "dunst"
    else notificationBackend;
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    domains.notifications.backend = notificationBackend;
    home.packages = with pkgs; [ libnotify ] ++ [ angstNotify ];
    services.dunst.enable = effectiveBackend == "dunst";
    services.dunst.settings = lib.mkIf (effectiveBackend == "dunst") {
      global = {
        width = 350;
        height = 300;
        offset = "20x20";
    ...
    services.mako.enable = effectiveBackend == "mako";
    services.mako.settings = lib.mkIf (effectiveBackend == "mako") {
