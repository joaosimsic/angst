...
| `/domains/` | Features — the unit of configuration (27 features / 17 categories), each with optional `home.nix`/`system.nix` sides |
...
- **Domains** — The unit of configuration: `domains/<category>/<name>/` with `meta.nix` (package + XDG target), optional `home.nix` (home-manager), `system.nix` (NixOS), `render.nix` (theme-aware config generator), and `config/`. A feature may be user-space, system-space, or both. Auto-discovered and turned into modules by `lib/domains/`.
...
- **Adding a domain**: create `domains/<category>/<name>/meta.nix`, optionally `home.nix`, `system.nix`, `render.nix`, `config/`. Enable via a profile or host extra modules.
