...
hosts/<name>/default.nix
  ├─ system, theme, user, monitors
  ▼
lib/build/mkHome.nix
  ├─ themesLib (from /themes/default.nix)
  ├─ domainsLib (from /lib/domains/default.nix)
  ├─ injects theme, userConfig, monitors as specialArgs
  ▼
...
- **angst CLI** — Shell script (`angst render`/`angst watch`) for hot-reload
- **Dev shells** — `safe` (neovim + parsers + LSPs + formatters) and `dev` (safe + Rust/QEMU/tools)
- **allToolchainPackages/allGrammars** — Aggregated from all 18 toolchains
...
| `/lib/build/mkHome.nix` | Home-manager profile constructor |
| `/lib/build/mkHost.nix` | NixOS system constructor |
| `/lib/flake/default.nix` | Flake outputs: domain renders, checks, dev shells |
| `/lib/flake/shared.nix` | Shared infra: angst CLI, shells, toolchains, treesitter |
...
- **`/lib/flake/shared.nix`** — Cross-cutting utilities: angst CLI, toolchain aggregation, tree-sitter, dev shell PATH construction. Change dev shell contents here.
...
- **`/lib/build/mkHome.nix`** — Core home-manager profile builder. Passes `themesLib`, `hostTheme`, `userConfig`, `monitors` as `extraSpecialArgs` to all modules. Adding a new `extraSpecialArgs` key here propagates to all domain `render.nix` functions.
- **`/lib/build/mkHost.nix`** — NixOS system builder. Wires home-manager into NixOS, imports capabilities, generates NixOS domain modules.
