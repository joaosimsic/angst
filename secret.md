...
The work-key ftp config is decrypted into `~/.secrets/ftp/<name>.conf` (0600) at
home activation — exactly like `opencode-go-key` — using the host's work key.
No re-encryption; the repo file stays work-key-only.

Layout:
- `~/.secrets/opencode-go-key` — personal sops secret (decrypted by sops-nix).
- `~/.secrets/projects/` — decrypted project store (`{personal,work}/<id>/…`).
- `~/.secrets/ftp/<name>.conf` — work-key rclone config (decrypted at activation).
...
- Mount service receives `configFile = ".secrets/ftp/<name>.conf"` (plaintext).

### A4. `runtime/ftp-mount.nix`
- Read `$HOME/.secrets/ftp/<name>.conf` directly.
...
- `modules/vm/vm-profile.nix`: `injectWorkAgeKey` defaults to true so the vm
  always gets `work-keys.txt`; drop the host-decl finagling.
...
- After switch on mint: `~/secrets` gone; `~/.secrets/ftp/ftp-server.conf` (0600)
  exists; `systemctl --user start angst-ftp-ftp-server` still mounts.

## Risk
Plaintext rclone config now rests at `~/.secrets/ftp/ftp-server.conf` (0600, dir 700)
